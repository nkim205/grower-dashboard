...
const COUNTIES_GEOJSON_URL = "/counties.geojson"
...
export default function Map3D({ searchTerm }) {
  ...
  useEffect(() => {
    ...
    const loadCSV = async(url) => {
        ...
        if (processedText.charCodeAt(0) === 0xfeff) processedText = processedText.slice(1);
        ...
        for (const row of rows) {
            ...
            byFips[fipsKey] = {
                ...
                saifi: Number(rowObj.SAIFI || 0),  //I added ts
                fips: fipsKey,
                state: rowObj.State,
            };
        }
  ...
  useEffect(() => {
    ...
    const countyMatches = (f) => {
      ...
      return (
        countyName.toLowerCase().includes(q) ||
        full.includes(q)
      )
    }
        ...
        new GeoJsonLayer({
          ...
          onHover: (info) => {
            ...
            setHoverInfo({
              ...
              saidi: m?.saidi ?? null,
              saifi: m?.saifi ?? null,  //I added ts too
              fips: m?.fips ?? null,
            })
          },
        })
    ...
    <div ref={containerRef} className="relative w-full h-full">
      ...
      {hoverInfo && (
        ...
        >
          ...
          {hoverInfo.saidi != null && (
            <div>
              SAIFI: {hoverInfo.saifi.toFixed(2)}
              {hoverInfo.mode === "state" && " (avg)"}
            </div>
          )}
        </div>
